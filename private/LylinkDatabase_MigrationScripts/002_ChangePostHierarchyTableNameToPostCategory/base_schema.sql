...
CREATE TABLE post_hierarchy (
  ...
  CONSTRAINT fk_post_hierarchy_parent FOREIGN KEY (parentId) REFERENCES post_hierarchy (categoryId) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
CREATE TABLE posts (
  ...
  CONSTRAINT fk_posts_parent FOREIGN KEY (parentId) REFERENCES post_hierarchy (categoryId) ON DELETE SET NULL ON UPDATE CASCADE
